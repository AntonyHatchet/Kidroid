/**
 * Created by nikolay_ivanisenko on 12.05.2015.
 */
var filterTable = function(b, c) {
    var d = b.getElementsByTagName("TR"),
        e = {},
        a;
    for (a in c) c.hasOwnProperty(a) && (e[a] = c[a] instanceof filterTable.Filter ? c[a] : new filterTable.Filter(c[a]), e[a]._setAction("onchange", function() {
        var a, c, b;
        for (c = 0; c < d.length; c += 1)
            for (b in a = d.item(c), e)
                if (e.hasOwnProperty(b))
                    if (!1 === e[b].validate(a.children[b].innerText)) {
...
filterTable.Filter = function(b, c, d) {
    ...
    this.validate = function(c) {
        for (var a = 0; a < this.filters.length; a += 1)
            if (!1 === this.__validate(c, this.filters[a], a)) return !1
    };
    this.__validate = function(b, a, f) {
        if ("undefined" !== typeof c) return c(b, this.filters, f);
